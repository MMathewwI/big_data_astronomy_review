...
According to Gunn et al. (2006), the SDSS camera contains “30 2048 x 2048 Scientific Imaging Technologies Charge-Coupled Devices (CCDs) and 24 2048 x 400 CCDs” \citep{gunn25TelescopeSloan2006}. 
...
That measurement is then converted into a digital value and stored as a pixel, forming an image \citep{lesserSummaryChargeCoupledDevices2015}.

Another major innovation that enables the SDSS to collect data is the pair of fiber-fed double spectrographs, which record imaging data across wavelengths from 3800 to 9200~\AA\ and at field angles between 0 and 90$^{\circ}$ \citep{gunn25TelescopeSloan2006}. 
...
Based on the data from \citep{gunn25TelescopeSloan2006}, both of these quantities remain close to zero for most wavelengths and field angles, except between roughly 5300 and 6500~\AA, which demonstrates the high optical accuracy of the SDSS spectrographs.

The combination of these two innovations alongside others help the 2.5 m telescope collect data at a rate of about 20Gb/hr \citep{luptonSDSSImagingPipelines2007}.

\textbf{(2) The Irénée du Pont Telescope:}  
Unlike the 2.5 m telescope, the Du Pont Telescope does not rely on CCDs to collect data. According to Bowen's 1973 paper, the telescope is described as a modified Ritchey-Chrétien design with Gascoigne correctors \citep{bowenOpticalDesign40in1973}.
The Du pont telescope uses a 100-inch primary mirror. Approximately 40\% of the light is reflected to the secondary mirror, obtaining only a 16\% loss of light at that stage. \citep{bowenOpticalDesign40in1973}
...
Another technology the du Pont Telescope applies is a Gascoigne corrector plate. The plate helps with data collection. The Gasciogne corrector plate is able to be moved, which can help optimize the collection of light in a wanted wavelength \citep{bowenOpticalDesign40in1973}.
Given a seperation of 1000 mm from the end of the corrector plate to the focus gives an image with a minimized astigmatism for a refractive index of n = 1.47 \citep{bowenOpticalDesign40in1973}.
...
The last technology the du Pont Telescope uses is conical baffles. The reason for this is to promote shielding in the telescope \citep{bowenOpticalDesign40in1973}.
...
As explained in the Bowen paper, the conic baffles are "located in the space between the incoming beam as it appraoches the primary and the return beam from the secondary to the plate" \citep{bowenOpticalDesign40in1973}.
Theoretically, the conic baffles have the disatvantage of producing a diffraction pattern. However, as explained by Bowen, this should not majorly affect the images of stars \citep{bowenOpticalDesign40in1973}. 
...
The NMSU telescope uses a camera that has a 2048 x 2048 CCD. The camera is controled by a linux computer, which is connected by fiber optic cables \citep{holtzmanNMSU1Telescope2010}.

The data collection of the NMSU telescope is almost fully automated using C++, the only time it is not is when engineering is being done on the telescope \citep{holtzmanNMSU1Telescope2010}.
...
When the sky becomes dark enough, the telescope initiates its program. The telescope goes through the list and observes said objects \citep{holtzmanNMSU1Telescope2010}.
...
This system ensures that data are processed and stored efficiently without any loss of precision. \citep{gunn25TelescopeSloan2006}. 
...
The pipeline starts by choosing an antenna as a reference point for the other antennas \citep{ratcliffeSDPPipelinesOverview2021}. This process is done by collecting the first scan of an antenna. 
The data is then averaged and fourier transformed. After that, it is the ratio called the peak-to-noise ratio is measured. The peak-to-noise ratio is the ratio of the maximum of the fourier transformed data and the rms noise at a distance far from the peak \citep{ratcliffeSDPPipelinesOverview2021}.
After tha the process repeats for every antenna and the one with the highest median with respect to every baseline for the peak-to-noise ratio is chosen as the reference antenna \citep{ratcliffeSDPPipelinesOverview2021}.
This antenna will have all of its calibration solutions set to zero \citep{ratcliffeSDPPipelinesOverview2021}. 


The purpose of the calibration pipeline is to ensure that the reference antenna and the other antennas by extension are callibrated. At the start of every observation, the reference antenna is evaluated based on certain flags, such as data loss \citep{ratcliffeSDPPipelinesOverview2021}. 
If 80\% or more flags are pinged, a new reference antenna is determined based on the aforementioned process \citep{ratcliffeSDPPipelinesOverview2021}.
...
The aforementioned \texttt{MFImage} task uses joint frequency deconvolution to handle wide-band effects in wide-band images \citep{ratcliffeSDPPipelinesOverview2021}. 
...
Additionally, there are facets with around 1 degree that  use the SUMSS or NVSS catalogue for a radius of 2.5 from the center of the phase \citep{ratcliffeSDPPipelinesOverview2021}.
...
Lastly, the data follows a cycle in order to be updated as described in the figure below. 
...
%The South African Radio Astronomy Observatory (SARAO) has expressed that MeerKAT data is to be open source with adequate acknowledgement through two steps \citep{camiloDOCUMENTDISTRIBUTION2024}. 
%First and foremost, the SARAO team emphasize that all MeerKAT data used must come from the MeerKAT ADS Library\footnote{\url{https://ui.adsabs.harvard.edu/public-libraries/wmc9yO6IQ3mUZCPx7MQRxg}} \citep{camiloDOCUMENTDISTRIBUTION2024}. 
%The other step is in any publication that contains MeerKAT data, the author must state the following statement verbatim: "The MeerKAT telescope is operated by the South African Radio Astronomy Observatory, which is a facility of the National Research Foundation, an agency of the Department of Science and Innovation." \citep{camiloDOCUMENTDISTRIBUTION2024}.

%The SARAO also encourages the user input. The SARAO advices users to configure with the data, creating data products \citep{camiloDOCUMENTDISTRIBUTION2024}. The data products can then be released into the MeerKAT archive by contacting the SARAO, which only serves to better the MeerKAT \citep{camiloDOCUMENTDISTRIBUTION2024}.
%Another policy the SARAO implements with the MeerKAT is the use of feedback. The SARAO has developed a help desk \footnote{\url{https://commons.datacite.org/doi.org?query=client.uid%3Awhno.ljncxe&resource-type=dataset}} from which users can ask general questions \citep{camiloDOCUMENTDISTRIBUTION2024}. If users have questions on policies of the MeerKAT, they are advised to communicate them to the SARAO chief scientist \citep{camiloDOCUMENTDISTRIBUTION2024}.
%The use of user input has also caused the creation of the SARAO Users Committee, which serves to voice the opinions of users towards the SARAO MD \citep{camiloDOCUMENTDISTRIBUTION2024}.
